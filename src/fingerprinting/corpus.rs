...
use super::{Analyzer, BannerRegexAnalyzer, PortContext, ServiceVerdict};
...
fn prefilter_never_drops_a_matching_signature() {
    ...
    assert_eq!(violations, 0, "prefilter dropped {violations} matching signature(s)");
}
...
fn golden_cases_resolve_end_to_end() {
    ...
    for case in cases {
        let evidence = BannerRegexAnalyzer
            .analyze(&PortContext { port: case.port }, &[case.response.to_string()]);
        let verdict = ServiceVerdict::resolve(evidence);

        assert_eq!(verdict.service.as_deref(), Some(case.service), "service for {:?}", case.response);
        if let Some(product) = case.product {
            assert_eq!(verdict.product.as_deref(), Some(product), "product for {:?}", case.response);
        }
        if let Some(version) = case.version {
            assert_eq!(verdict.version.as_deref(), Some(version), "version for {:?}", case.response);
        }
    }
}
...
fn non_standard_port_is_identified_via_global_fallback() {
    ...
    let evidence =
        BannerRegexAnalyzer.analyze(&PortContext { port }, &["SSH-2.0-OpenSSH_9.6p1".to_string()]);
    let verdict = ServiceVerdict::resolve(evidence);
