...
pub fn get_fingerprints() -> &'static [CompiledService] {
    FINGERPRINTS.get_or_init(|| {
        ...
        defs.into_iter()
            .map(|def| {
                let matches = def.r#match.iter().filter_map(|m| {
...
pub async fn fingerprint_tcp(mut stream: TcpStream, mut port: Port) -> Port {
    ...
    for srv in fingerprints {
        if responses.is_empty() && srv.def.service.default_ports.contains(&port.number) {
            if let Some(probe) = srv.def.probe.first() {
        ...
        if !responses.is_empty() {
            for m in &srv.matches {
                if let Some(caps) = m.pattern.captures(&responses) {
                    let mut info = m.product.clone().unwrap_or_else(|| srv.def.service.name.clone());
                    
                    if let Some(group_idx) = m.version_group {
                        if let Some(ver) = caps.get(group_idx as usize) {
    ...
    if port.service_info.is_none() && !responses.is_empty() {
        let clean: String = responses.chars()
