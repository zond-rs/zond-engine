...
pub fn create_packet(src_addr: &IpAddr, dst_addr: &IpAddr, src_port: u16, dst_port: u16) -> anyhow::Result<Vec<u8>> {
    let mut buffer: Vec<u8> = vec![0u8; MIN_TCP_HDR_LEN];
    {
        let mut tcp: MutableTcpPacket = MutableTcpPacket::new(&mut buffer).context("creating tcp packet")?;
