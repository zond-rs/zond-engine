...
/// RAII wrapper for a network namespace infrastructure used in integration tests.
///
/// It creates a new network namespace, a veth pair, and sets up IP addresses
/// to simulate a real network environment without touching the host's primary stack.
...
impl NetnsContext {
    /// Creates a new network namespace with the given suffix.
    /// Returns None if root privileges are missing or 'ip' command fails.
...
pub fn run_ns_cmd(ns: &str, cmd: &str, args: &[&str]) -> bool {
